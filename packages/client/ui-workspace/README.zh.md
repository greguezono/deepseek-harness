...
- **没有 Session 删除与取消归档控件**：视图选项的**显示已归档**开关会在每个展开的 Workspace 下列出其已归档会话（仅限分组模式，最新 5 条，其余收在展开控件后），并以只读方式打开；但没有任何入口可以取消归档，平铺列表与内容搜索仍然不含已归档行，删除 Workspace 注册记录也不会删除 Session。

...
describe('workspaces action face', () => {
  it('records every IWorkspaces verb with inert defaults and honors stubs', async () => {
    ...
    ws.setAllowArchivedCurrent(true)
    expect(ws.calls.map(c => c.method)).toEqual(
      ['create', 'create', 'pickDirectory', 'rename', 'delete', 'openPath', 'insertBefore', 'insertSessionBefore', 'archiveSession', 'setAllowArchivedCurrent'])
    ...
    ws.stub('setAllowArchivedCurrent', setAllowArchivedCurrent)
    ...
    expect(ws.list.getSnapshot().archivedSessionIds).toEqual(['s1'])
    ws.setAllowArchivedCurrent(false)
    expect(setAllowArchivedCurrent).toHaveBeenCalledWith(false)
    await runtime.dispose()
  })
})

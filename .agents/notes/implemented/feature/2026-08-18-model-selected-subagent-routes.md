...
- Shipped fork tools omit model-facing route fields so the inherited conversation prefix remains eligible for KV Cache reuse.
- An enabled Session uses its recorded default when a call omits the route; a route change without an explicit effort uses the selected model's default.
